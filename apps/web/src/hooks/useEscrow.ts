...
import { parseUnits, stringToHex, toBytes } from "viem";
...
const ESCROW_CONTRACT_ADDRESSES: Record<number, `0x${string}`> = {
  42220: (process.env.NEXT_PUBLIC_ESCROW_ADDRESS_MAINNET || "0x0000000000000000000000000000000000000000") as `0x${string}`, // Mainnet
  44787: (process.env.NEXT_PUBLIC_ESCROW_ADDRESS_ALFAJORES || "0x0000000000000000000000000000000000000000") as `0x${string}`, // Alfajores Testnet
  11142220: (process.env.NEXT_PUBLIC_ESCROW_ADDRESS_SEPOLIA || "0x0000000000000000000000000000000000000000") as `0x${string}`, // Sepolia Testnet
};
...
/**
 * Converts a string taskId into a 32-byte hash (bytes32 hex string) for solidity storage
 */
export const formatTaskIdToBytes32 = (taskId: string): `0x${string}` => {
  // Pad or truncate task ID to fit bytes32 exactly
  return stringToHex(taskId.slice(0, 31).padEnd(32, "\0")) as `0x${string}`;
};
